// !!! Yo, you should read this !!!
// jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj

// I. An ADS-B frame is 112 bits long and consists of five main parts, shown as follows:
// +----------+----------+-------------+------------------------+-----------+
// |  DF (5)  |  CA (3)  |  ICAO (24)  |         ME (56)        |  PI (24)  |
// +----------+----------+-------------+------------------------+-----------+

// II. Or represented in hex as follors:
// *8D4840D6202CC371C32CE0576098;
// * in is start delim. and ; is end delim

// III. Civil aircraft ADS-B message starts with the Downlink Format 17.
// It corresponds to 10001 in binary for the first 5 bits.
// Bits 6–8 indicated the transponder capability.
// After that, the 24-bit transponder code (also known as ICAO code) is included.
// The last two segments are 56-bit payload and 24-bit parity.
// Table bellow lists the key information of an ADS-B message.
// +----------+-------------+-----------------+-------------------------------+
...
// IV. ADS-B Type Code and content:
...
var ERR_PACKET_INVALID = (ERR_PACKET_INVALID = 0);
var ERR_PACKET_TOO_SHORT = (ERR_PACKET_TOO_SHORT = -1);
var ERR_PACKET_TOO_LONG = (ERR_PACKET_TOO_LONG = -2);
...
decodePacket = function (data) {
  // ⚠️  WARNING ⚠️
  // Server packet does not contain the * and ; markers
  data = `*${data};`;
  stats.packets++;
  var len = data.length;
  ...
  for (var i = 1; i < len - 1; i += 2) {
    // Skip beginning and end (* and ;), go 2 chars at a time
  ...
  // msg.type will contain only first 3 bytes of bytes
  // example: value 141 will become 17
  msg.type = bytes[0] >> 3;
  if (DEBUG)
  ...
  if (!msg.crc_ok) {
    ...
    if (DEBUG)
      console.log(
        "!!!!!!!!!!!!!!! CRC check failed: " + msg.crc + " vs " + crc2,
      );
  ...
  if (DEBUG)
    console.log(
      "CA " + msg.ca + ": " + this.responderCapabilitiesToString(msg.ca),
    );
  ...
  if (msg.type == 17) {
    ...
    if (DEBUG)
      console.log(
        "Extended Squitter Type " +
          msg.metype +
          ", Subtype: " +
          msg.mesub +
          ": " +
          this.meTypeToString(msg.metype, msg.mesub),
      );
  }
  ...
  console.log(
    "Flight Status " + msg.fs + ": " + this.flightStatusToString(msg.fs),
  );
  ...
  if (msg.msgtype == 17) {
    ...
    if (msg.metype >= 1 && msg.metype <= 4) {
      ...
      msg.raw_latitude =
        ((bytes[6] & 3) << 15) | (bytes[7] << 7) | (bytes[8] >> 1);
      ...
      if (msg.mesub == 1 || msg.mesub == 2) {
        ...
        msg.velocity = sqrt(
          msg.ns_velocity * msg.ns_velocity + msg.ew_velocity * msg.ew_velocity,
        );
        if (msg.velocity) {
...
modesChecksum = function (bytes, num_bits) {
  var crc = 0;
  var offset =
    num_bits == MODES_LONG_MSG_BITS
      ? 0
...
messageTypeToString = function (type) {
  switch (type) {
    case 0:
...
responderCapabilitiesToString = function (ca) {
  switch (ca) {
    case 0:
...
meTypeToString = function (metype, mesub) {
  var mename = "Unknown";
...
flightStatusToString = function (fs) {
  switch (fs) {
    case 0:
...
decodeAC13Field = function (bytes) {
...
decodeAC12Field = function (bytes) {
  var q_bit = bytes[5] & 1;
